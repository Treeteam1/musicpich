...
if(isset($_POST['page']) && !empty($_POST['page'])){
    $page = $_POST['page'];
    $limit = 5; // Количество элементов, которые нужно загрузить на каждой странице
    $offset = ($page - 1) * $limit;

    // Выполните SQL-запрос, чтобы получить данные из базы данных с использованием LIMIT и OFFSET
    $db = new App\Database();
    $stmt = $db->conn->prepare("SELECT * FROM `musics` WHERE id > :lastId LIMIT :limit OFFSET :offset");
    ...
    // После выполнения запроса, отформатируйте данные и верните их обратно на клиентскую сторону
    // Пример:
